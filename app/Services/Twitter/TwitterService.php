...
class TwitterService
{
    ...
    public function __construct()
    {
        ...
        $this->oauth_access_token = config('services.twitter.access_token');
        $this->oauth_access_token_secret = config('services.twitter.access_token_secret');
        $this->consumer_key = config('services.twitter.consumer_key');
        $this->consumer_secret = config('services.twitter.consumer_secret');
    }
    ...
    public function getUserTimelineFromJson()
    {
        if(!Storage::exists('twitter/user_timeline.json'))
        {
            return false;
        }

        return collect(json_decode(Storage::get('twitter/user_timeline.json')))->take(3);
